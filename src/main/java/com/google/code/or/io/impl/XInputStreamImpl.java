...
public class XInputStreamImpl extends InputStream implements XInputStream {
	...
	public UnsignedLong readUnsignedLong(XChecksum checksum) throws IOException {
		...
		else if(v == 252) return UnsignedLong.valueOf(readInt(2, checksum));
		else if(v == 253) return UnsignedLong.valueOf(readInt(3, checksum));
		else if(v == 254) return UnsignedLong.valueOf(readLong(8, checksum));
		else throw new RuntimeException("assertion failed, should NOT reach here");
	}
	 ...
	 * @add
	 * */
	public StringColumn readFixedLengthString(final int length, XChecksum checksum) throws IOException {
	 ...
	 * */
	@Override
	public int read(final byte b[], final int off, final int len,XChecksum checksum) throws IOException {
		if(this.readLimit > 0 && (this.readCount + len) > this.readLimit) {
	 ...
	 * 
	 */
	private void doFill() throws IOException {//with no need for checksum at this step (in read step!)
		this.head = 0;
	 ...
	 * @add
	 * */
	private int doRead(final byte[] b, final int off, final int len, XChecksum checksum) throws IOException {
		int total = len;
		int index = off;
		while(total > 0) {
			final int available = this.tail - this.head;
