...
public class AuthenticatorImpl implements Transport.Authenticator {
  ...
  //
  public static final int DEFAULT_CAPABILITIES = (MySQLConstants.CLIENT_LONG_FLAG
      | MySQLConstants.CLIENT_PROTOCOL_41 | MySQLConstants.CLIENT_SECURE_CONNECTION);
  ...
  public void login(Transport transport) throws IOException {
    ...
    LOGGER.info("start to login, user: {}, host: {}, port: {}",
    ...
    s.writeBytes(MySQLUtils.password41OrLater(this.password.getBytes(this.encoding), ctx
        .getScramble().getBytes(this.encoding)));
